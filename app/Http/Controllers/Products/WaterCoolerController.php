...
use App\Models\Products\WaterCooler;
...
class WaterCoolerController extends Controller
{
    ...
    {
        $products = WaterCooler::all();
        return view('Products.WaterCoolers',['products'=>$products]);
    }


    public function show($id){
        $product = WaterCooler::query()->find($id);
        return view('Products.WaterCooler_item',['product'=>$product]);
    }
    ...
    public function addtobasket(Request $res){
        ...
        $Product = WaterCooler::query()->find($res->Product_id);
